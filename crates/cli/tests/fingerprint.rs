...
//! (`convert_v4` → serialize → `Engine::from_json` → `filter`/`grid`) over
...
use syllabus_core::{Engine, Filters, RawCourse, convert_v4};
...
/// Build an engine the way the CLI does: parse raw → `convert_v4` → serialize →
...
fn engine_from_fixture() -> Engine {
    ...
    .data;
    let json = serde_json::to_string(&data).expect("serialize v4 payload");
