...
| WASM boundary shapes | `wasm-pack test --node` | CI `wasm` | `just test-wasm` |
...
| Mutation (Rust: core + tested cli logic) | `cargo-mutants` | weekly advisory + release PR required | `just mutants` |
| Mutation (web lib) | Stryker | weekly advisory + release PR required | `just stryker` |
...
- **Mutation testing**: measures whether the suite *kills* injected bugs and
  surfaced a real gap once (an arc test that ignored the dash gap). Weekly full
  and ordinary-PR diff runs provide advisory feedback; a release-please PR must
  pass full Rust and Web mutation jobs in the same `Required gate` as every
  other release check. Rust scope (all of `syllabus-core` plus the tested
  `syllabus-cli` parsers/converters) is declared in `.cargo/mutants.toml`; the
  web scope in `web/stryker.conf.json`. Both exclude data tables and the
  worker/network/IO code, which unit/E2E/integration layers cover.
...
- `just check` is the complete local static/unit/security gate.
  `just release-check` runs that gate first, then builds the strict real-data
  artifact and adds browser E2E, payload/query budgets, and Lighthouse. Run
  `just setup` once to install every pinned tool and Playwright browser.
